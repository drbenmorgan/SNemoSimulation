...
namespace snemo {
  ...
  namespace g4 {
    ...
    bool
    magnetic_field::has_name() const
    {
      return !_name_.empty();
    }

    void
    magnetic_field::set_name(const std::string& name_)
    {
      _name_ = name_;
      return;
    }
    ...
    const std::string&
    magnetic_field::get_name() const
    {
      return _name_;
    }

    void
    magnetic_field::set_field_check_pos_time(bool c_)
    {
    ...
    bool
    magnetic_field::is_field_check_pos_time() const
    {
      return _field_check_pos_time_;
    }

    bool
    magnetic_field::is_initialized() const
    {
      return _initialized_;
    }

    bool
    magnetic_field::has_field() const
    {
      return _field_ != 0;
    }

    bool
    magnetic_field::has_mag_field() const
    {
      return _field_ != 0;
    }

    void
    magnetic_field::set_mag_field(const emfield::base_electromagnetic_field& mf_)
    {
      _field_ = &mf_;
      return;
    }
    ...
    void
    magnetic_field::set_field(const emfield::base_electromagnetic_field& mf_)
    {
      _field_ = &mf_;
      return;
    }
    ...
    bool
    magnetic_field::is_active() const
    {
      return has_mag_field();
    }

    void
    magnetic_field::_set_defaults()
    {
    ...
    void
    magnetic_field::initialize(const datatools::properties& config_)
    {
      DT_THROW_IF(_initialized_,
                  std::logic_error,
                  "Geant4 magnetic field '" << get_name() << "' is a lready initialized !");
      ...
      if (!has_field()) {
        ...
        if (config_.has_key("mode")) {
          ...
          if (mode_str == "standalone") {
            ...
            DT_THROW_IF(true,
                        std::logic_error,
                        "Invalid value '" << mode_str
                                          << "' for 'mode' property in Geant4 magnetic field '"
                                          << get_name() << "' !");
          }
        } else {
          ...
          DT_THROW_IF(true,
                      std::logic_error,
                      "Missing 'mode' property in Geant4 magnetic field '" << get_name() << "' !");
        }
          ...
          if (!geomtools::is_valid(_standalone_constant_field_)) {
            if (config_.has_key("standalone_constant_mag_field")) {
              ...
              DT_THROW_IF(smg.size() != 3,
                          std::logic_error,
                          "Invalid dimension for vector of constant magnetic field coordinates in "
        ...
        /*
          ...
          if (_geom_manager_ == 0) {
          std::ostringstream message;
          message << "snemo::g4::magnetic_field::initialize: "
    ...
    // G4 interface:
    void
    magnetic_field::GetFieldValue(const double position_[4], double* b_field_) const
    {
      DT_LOG_TRACE(_logprio(),
                   "Entering GetFieldValue for Geant4 magnetic field '" << get_name() << "'...");
      // DT_LOG_TRACE(datatools::logger::PRIO_ALWAYS, "Entering GetFieldValue for Geant4 magnetic
      // field '" << get_name() << "'...");
      DT_THROW_IF(!_initialized_,
                  std::logic_error,
                  "Geant4 magnetic field '" << get_name() << "' is not initialized !");
      ...
      if (_field_ != 0) {
        DT_LOG_TRACE(_logprio(),
                     "Compute magnetic field for Geant4 magnetic field '" << get_name() << "'...");
        // DT_LOG_TRACE(datatools::logger::PRIO_ALWAYS, "Compute magnetic field for Geant4 magnetic
        // field '" << get_name() << "'...");
        ...
        DT_LOG_TRACE(_logprio(),
                     "Geant4 magnetic field '"
                       << get_name() << "' is : " << the_b_field / CLHEP::gauss << " gauss");
      ...
      DT_LOG_TRACE(_logprio(),
                   "Exiting GetFieldValue for Geant4 magnetic field '" << get_name() << "'.");
      return;
    }

    void
    magnetic_field::dump(std::ostream& out_) const
    {
      out_ << "magnetic_field:" << std::endl;
      out_ << "|-- Name                   : '" << _name_ << "'" << std::endl;
      if (geomtools::is_valid(_standalone_constant_field_)) {
        out_ << "|-- Standalone constant magnetic field : "
             << _standalone_constant_field_ / CLHEP::gauss << " gauss" << std::endl;
      } else {
