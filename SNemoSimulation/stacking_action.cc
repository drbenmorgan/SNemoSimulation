...
namespace snemo {
  ...
  namespace g4 {
    ...
    void
    stacking_action::initialize(const datatools::properties& config_)
    {
    ...
    stacking_action::ClassifyNewTrack(const G4Track* a_track)
    {
      ...
      if ((_killer_volume_names_.size() > 0) && (_killer_volumes_.size() == 0)) {
        ...
        for (int i = 0; i < (int)_killer_volume_names_.size(); i++) {
          ...
          DT_THROW_IF(g4_volume_log == 0,
                      std::logic_error,
                      "Logical volume '" << g4_log_name << "' does not exists !");
          DT_LOG_NOTICE(_logprio(),
                        "Add a secondary particle killer logical volume '" << g4_log_name << "'.");
      ...
      if (_kill_secondary_particles_ && (a_track->GetParentID() > 0)) {
        ...
        if (!kill) {
          if (g4_volume_log != 0) {
            ...
            if (g4_material != 0) {
              ...
              if (find(_killer_materials_.begin(), _killer_materials_.end(), g4_material) !=
        ...
        if (!kill) {
          if (g4_volume_log != 0) {
            ...
            if (find(_killer_volumes_.begin(), _killer_volumes_.end(), g4_volume_log) !=
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::stacking_action, ocd_)
{
  ...
  ocd_.set_class_documentation(
    "This is Geant4 simulation engine embedded stacking action.      \n");
...

  {
    ...
    cpd.set_name_pattern("kill_particles.volumes")
      .set_terse_description(
        "The list of logical volumes where secondary particle killing is activated")
...

  {
    ...
    cpd.set_name_pattern("kill_particles.materials")
      .set_terse_description("The list of materials of which logical volumes are made and where "
                             "secondary particle killing is activated")
      .set_traits(datatools::TYPE_STRING, configuration_property_description::ARRAY)
      .set_mandatory(false)
      .set_triggered_by_flag("kill_particles")
