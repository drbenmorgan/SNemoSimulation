...
namespace snemo {
  ...
  namespace g4 {
    ...
    void
    detector_construction::_set_default()
    {
    ...
    bool
    detector_construction::is_initialized() const
    {
      return _initialized_;
    }

    bool
    detector_construction::has_geometry_manager() const
    {
      return _geom_manager_ != 0;
    }

    void
    detector_construction::set_geometry_manager(const geomtools::manager& gm_)
    {
    ...
    detector_construction::reset()
    {
      ...
      for (std::map<std::string, G4VisAttributes*>::iterator i = _vis_attributes_.begin();
    ...
    void
    detector_construction::initialize(const datatools::properties& config_)
    {
      ...
      DT_THROW_IF(_SHPF_config_.empty(),
                  std::logic_error,
                  "Missing property '"
      ...
      if (config_.has_key("sensitive.detectors")) {
        ...
        for (unsigned int i = 0; i < list_of_sensitive_detectors.size(); ++i) {
          ...
          config_.export_and_rename_starting_with(
            _SD_params_.grab(SD_category).grab_properties(), key_oss, new_key_oss);
        } // for
      ...
      if (_using_em_field_) {
        ...
        if (config_.has_key("electromagnetic_field.miss_distance.unit")) {
          std::string md_unit_str =
            config_.fetch_string("electromagnetic_field.miss_distance.unit");
          ...
          // Obsolete:
          DT_LOG_WARNING(datatools::logger::PRIO_ALWAYS,
                         "Configuration property 'magnetic_field.miss_distance.unit' is obsolete, "
                         "use 'electromagnetic_field.miss_distance.unit'!");
        ...
        if (_emfield_geom_plugin_name_.empty()) {
          ...
          for (dict_type::const_iterator ip = plugins.begin(); ip != plugins.end(); ip++) {
            const std::string& plugin_name = ip->first;
            if (geo_mgr.is_plugin_a<emfield::emfield_geom_plugin>(plugin_name)) {
              emfield_geom_plugin_name = plugin_name;
              DT_LOG_DEBUG(_logprio(),
                           "Found EM field plugin = '" << emfield_geom_plugin_name << "'");
              break;
            }
          }
        }
        DT_THROW_IF(_emfield_geom_plugin_name_.empty(),
                    std::logic_error,
                    "No geometry EM field plugin name is provided!");
        // Access to a given plugin by name and type :
        DT_LOG_DEBUG(_logprio(),
                     "Trying to access to an EM field geometry plugin with name '"
                       << emfield_geom_plugin_name << "'");
        DT_LOG_DEBUG(_logprio(),
                     "A geometry plugin with name '"
                       << emfield_geom_plugin_name
                       << "' exists: " << geo_mgr.has_plugin(emfield_geom_plugin_name));
        DT_LOG_DEBUG(
          _logprio(),
          "Geometry plugin named '"
        ...
        if (_em_field_aux_.size() == 0) {
          // Try with the legacy (obsolete) prefix if the first one was sterile...
          config_.export_and_rename_starting_with(
            _em_field_aux_, "magnetic_field.", ""); // to be obsoleted
        }
      ...
      if (_using_biasing_) {
        ...
        DT_THROW_IF(_biasing_config_.empty(),
                    std::logic_error,
                    "Missing property '"
                      << "biasing"
                      << "' !");
    ...
    G4VPhysicalVolume*
    detector_construction::Construct()
    {
      DT_LOG_TRACE(_logprio(), "Entering...");
    ...
    G4VPhysicalVolume*
    detector_construction::_g4_construct()
    {
      DT_LOG_TRACE(_logprio(), "Entering...");
    ...
    bool
    detector_construction::has_em_field_manager() const
    {
      return _em_field_manager_ != 0;
    }

    bool
    detector_construction::has_mag_field_manager() const
    {
      return has_em_field_manager();
    }

    void
    detector_construction::set_mag_field_manager(
    ...
    void
    detector_construction::set_em_field_manager(
      const emfield::electromagnetic_field_manager& emf_mgr_)
    {
    ...
    {
      DT_THROW_IF(
        _em_field_manager_ == 0, std::logic_error, "No electromagnetic field manager is defined !");
      return *_em_field_manager_;
    }

    void
    detector_construction::_construct_biasing()
    {
    ...
    void
    detector_construction::_destroy_electromagnetic_field()
    {
      for (em_field_g4_data_type::iterator i = _em_field_g4_data_.begin();
           i != _em_field_g4_data_.end();
           i++) {
        DT_LOG_TRACE(_logprio(),
                     "Destroy G4 support for the '" << i->first
                                                    << "' geometry/EM-field association...");
        ...
        DT_LOG_TRACE(_logprio(),
                     "Destroy G4 support for the '" << i->first
                                                    << "' geometry/EM-field association... done.");
      }
    ...
    void
    detector_construction::_construct_electromagnetic_field()
    {
      ...
      DT_LOG_DEBUG(logging,
                   "Has EM field geom map : " << (has_em_field_manager() &&
      ...
      if (has_em_field_manager() && _em_field_manager_->has_geom_map()) {
        ...
        for (gma_type::const_iterator i = mfamap.begin(); i != mfamap.end(); i++) {
          const std::string& association_name = i->first;
          const gma_entry_type& gefa = i->second;
          if (!gefa.has_logvol()) {
            std::ostringstream message;
            message << "Missing G4 logical volume for geom/EM-field association '"
                    << association_name << "' !";
            if (_abort_on_error_) {
          ...
          if (g4_module_log == 0) {
            ...
            DT_LOG_NOTICE(logging,
                          " -> G4 logical volume '" << g4_log_name
                                                    << "' has an electromagnetic field labelled '"
                                                    << gefa.get_field_name() << "'.");
          }
          ...
          if (gefa.field->is_magnetic_field() && !gefa.field->is_electric_field()) {
            ...
            _em_field_aux_.export_and_rename_starting_with(
              field_config, association_name + ".", "");
            b_field->initialize(field_config);
            emf_working->set_g4_magnetic_field(b_field);
            emf_working->initialize(field_config);
            DT_LOG_NOTICE(logging,
                          "Initializing pure magnetic field with label '" << association_name
                                                                          << "'...");
          } else {
            ...
            emf_working->initialize(field_config);
            DT_LOG_NOTICE(logging,
                          "Initializing general electromagnetic field with label '"
                            << association_name << "'...");
          }
    ...
    detector_construction::_construct_sensitive_detectors()
    {
      ...
      for (std::map<std::string, std::string>::const_iterator i = lv_sd_associations.begin();
        ...
        if (!_SD_params_.has_key(sd_name)) {
          ...
          DT_LOG_WARNING(
            _logprio(),
            "Sensitive category '"
              << sd_name << "' does not exists but is supposed to be associated to logical volume '"
      ...
      for (geomtools::logical_volume::dict_type::const_iterator ilogical =
        ...
        sensitive_detector_dict_type::iterator found =
          _sensitive_detectors_.find(sensitive_category);
      ...
      datatools::logger::priority lp =
        datatools::logger::extract_logging_configuration(_SHPF_config_);
      ...
      for (SHPF_type::processor_dict_type::const_iterator iSHP = _SHPF_.get_processors().begin();
        ...
        if (_g4_manager_ != nullptr) {
          if (_g4_manager_->forbids_private_hits()) {
            if (boost::starts_with(hit_category, "_")) {
              DT_LOG_WARNING(_logprio(),
                             "SHPF: Sensitive detector '"
                               << processor->get_name()
                               << "' initialized with private hit category '" << hit_category
                               << "' is not used !");
              continue;
            }
          }
        ...
        const std::string& from_processor_sensitive_category =
          iSHP->second->get_sensitive_category();
        ...
        if (_sensitive_detectors_.find(from_processor_sensitive_category) !=
          ...
          DT_LOG_WARNING(_logprio(),
                         "SHPF: A sensitive detector '" << processor->get_name()
                                                        << "' with category '"
          ...
          DT_LOG_NOTICE(_logprio(),
                        "SHPF: Number of logical volumes associated to sensitive category '"
            ...
            for (geomtools::logical_volume::dict_type::const_iterator ilogical =
              ...
              if (checked_material) {
                if (std::find(logicals.begin(), logicals.end(), logical_name) == logicals.end()) {
                  ...
                  message << "SHPF: Associate the logical volume '" << logical_name
                          << "' with material '" << mat_name
                          << "' to the new sensitive detector with category '"
                          << from_processor_sensitive_category << "'";
          ...
          if (processor->get_auxiliaries().has_key("geometry.volumes.excluded")) {
            ...
            message << ") from the list of volumes to be associated to the new sensitive detector "
                       "from SHPF with category '"
                    << from_processor_sensitive_category << "'";
            DT_LOG_NOTICE(_logprio(), message.str());

            for (size_t ixm = 0; ixm < excluded_logicals.size(); ixm++) {
              logicals.erase(std::remove(logicals.begin(), logicals.end(), excluded_logicals[ixm]),
                             logicals.end());
            }
          }
          ...
          for (std::vector<std::string>::const_iterator ilogical = logicals.begin();
            ...
            if (g4_log == 0) {
              DT_LOG_WARNING(_logprio(),
                             "SHPF: No logical volume has been found '"
            ...
            // Local scope:
            {
              ...
              geomtools::logical_volume* mutable_log = const_cast<geomtools::logical_volume*>(&log);
    ...
    void
    detector_construction::set_materials_geom_plugin_name(const std::string& mpn_)
    {
      DT_THROW_IF(is_initialized(), std::logic_error, "Operation prohibited !");
    ...
    void
    detector_construction::set_emfield_geom_plugin_name(const std::string& fpn_)
    {
      DT_THROW_IF(is_initialized(), std::logic_error, "Operation prohibited !");
    ...
    void
    detector_construction::write_tmp_gdml_file()
    {
    ...
    void
    detector_construction::_set_user_limits()
    {
      DT_LOG_TRACE(_logprio(), "Entering...");
      ...
      for (std::map<std::string, std::string>::const_iterator i = targets.begin();
        ...
        if (i->second == "logical_volume") {
          ...
          */
          DT_LOG_NOTICE(_logprio(),
                        "Searching G4 logical volume '" << target_name.c_str() << "'...");
          g4_logical = G4LogicalVolumeStore::GetInstance()->GetVolume(target_name.c_str(), false);
          DT_THROW_IF(g4_logical == 0,
                      std::logic_error,
                      "Missing G4 logical volume with name '" << target_name
                                                              << "' for user limits !");
          ...
          DT_THROW_IF(
            g4_region == 0, std::logic_error, "Cannot find region named '" << target_name << "' !");
        }
        ...
        if (ulimits_used) {
          ...
          G4UserLimits* g4_user_limits = new G4UserLimits(
            max_step, max_track_length, max_track_time, min_track_ekin, min_track_range);
          _user_limits_col_.push_back(g4_user_limits);
          if (g4_logical) {
            g4_logical->SetUserLimits(g4_user_limits);
            DT_LOG_NOTICE(_logprio(),
                          "Installing user limits for G4 logical volume named '"
                            << target_name << "' with max step = " << max_step / CLHEP::mm
                            << " mm");
          } else if (g4_region) {
            g4_region->SetUserLimits(g4_user_limits);
            DT_LOG_NOTICE(_logprio(),
                          "Installing user limits for G4 region named '"
                            << target_name << "' with max step = " << max_step / CLHEP::mm
                            << " mm");
          }
        } else {
    ...
    void
    detector_construction::_construct_regions()
    {
      G4LogicalVolumeStore* g4_LV_store = G4LogicalVolumeStore::GetInstance();
      ...
      for (region_infos_dict_type::const_iterator i =
             _region_definitions_.get_region_infos().begin();
           i != _region_definitions_.get_region_infos().end();
           ++i) {
        ...
        for (std::set<std::string>::const_iterator j = the_region_info.get_logical_ids().begin();
             j != the_region_info.get_logical_ids().end();
             j++) {
          ...
          if (a_logical != nullptr) {
            ...
            DT_THROW(std::logic_error,
                     "No logical volume named '" << logical_volume_name << "' to be added in the '"
                                                 << the_region_name << "' region !");
          }
        }
      }
    ...
    void
    detector_construction::_set_visualization_attributes()
    {
      DT_LOG_TRACE(_logprio(), "Entering...");
      ...
      */
      for (geomtools::logical_volume::dict_type::const_iterator ilogical =
        ...
        if (g4_log != 0) {
          ...
          if (visible) {
            va->SetColor(
              G4Colour(color.GetRed(), color.GetGreen(), color.GetBlue(), display_opacity));
          }
          ...
          DT_LOG_DEBUG(_logprio(),
                       "Cannot find logical volume '" << log.get_name()
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::detector_construction, ocd_)
{
...

  {
    ...
    cpd.set_name_pattern("materials.plugin_name")
      .set_terse_description("The name of the materials plugin embedded in the geometry manager "
                             "for materials export in GDML")
      .set_traits(datatools::TYPE_STRING)
      .set_long_description(
        "Default value: empty (plugin is autodetected from the geometry manager)\n")
      .add_example("Use a plugin for materials with a specific name::             \n"
                   "                                                              \n"
...

  {
    ...
    cpd.set_name_pattern("hit_processor_factory.configs")
      .set_terse_description("The names of the files that contain the configurations\n"
      ...
      .add_example("Use two configuration files for back-end step hits processors:: \n"
                   "                                                              \n"
...

  {
    ...
    cpd.set_name_pattern("hit_processor_factory.config")
      .set_terse_description("The name of the file that contains the configurations \n"
      ...
      .set_long_description(
        ...
        "is provided.                                           \n"
        "                                                       \n")
      .add_example("Use only one configuration file for all step hits processors::  \n"
                   "                                                                \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.${sensitive.detectors}.hits_buffer_capacity")
      .set_terse_description("Pre-allocate the initial capacity of a buffer of step hits "
...

  {
    ...
    cpd.set_name_pattern("regions.${regions}.volumes.with_materials")
      .set_terse_description("The list of volumes with some given materials attached to a region "
...

  {
    ...
    cpd.set_name_pattern("magnetic_field.check_pos_time.${magnetic_field.associations}")
      .set_terse_description("Flag to force the checking of position/time while computing mag. "
                             "field for a given geom/mag. field association")
      .set_traits(datatools::TYPE_BOOLEAN)
      .set_mandatory(false)
      .set_default_value_boolean(false)
      .set_long_description(
