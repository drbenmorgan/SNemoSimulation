...
namespace snemo {
  ...
  namespace g4 {
    ...
    bool
    primary_generator::is_initialized() const
    {
      return _initialized_;
    }

    void
    primary_generator::set_run_action(snemo::g4::run_action& run_action_)
    {
    ...
    void
    primary_generator::set_event_action(snemo::g4::event_action& event_action_)
    {
      DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
    ...
    bool
    primary_generator::has_vertex_generator() const
    {
      return _vertex_generator_ != 0;
    }

    void
    primary_generator::set_vertex_generator(::genvtx::i_vertex_generator& vertex_generator_)
    {
      DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
    ...
    void
    primary_generator::set_event_generator(::genbb::i_genbb& event_generator_)
    {
      DT_THROW_IF(_initialized_, std::logic_error, "Operation prohibited ! Manager is locked !");
    ...
    size_t
    primary_generator::get_event_counter() const
    {
      return _event_counter_;
    }

    void
    primary_generator::reset_event_counter()
    {
      _event_counter_ = 0;
      return;
    }
    ...
    void
    primary_generator::_set_defaults()
    {
      reset_event_counter();
      return;
    }
    ...
    void
    primary_generator::_check()
    {
      if (has_vertex_generator()) {
        if (!_vertex_generator_->is_initialized()) {
          G4Exception("snemo::g4::primary_generator::_check",
                      "InitializationError",
                      RunMustBeAborted,
        ...
        DT_LOG_WARNING(_logprio(),
                       "No vertex generator is provided; "
    ...
    void
    primary_generator::initialize(const datatools::properties& config_)
    {
      if (is_initialized()) {
        G4Exception("snemo::g4::primary_generator::initialize",
                    "InitializationError",
                    RunMustBeAborted,
      ...
      if (config_.has_key("particle_names_map")) {
        ...
        for (size_t i = 0; i < names.size(); i++) {
          ...
          DT_THROW_IF(genbb_particle_name.empty(),
                      std::logic_error,
                      "Invalid genbb particle name extracted from mapping entry '" << mapping_entry
                                                                                   << "' !");
          DT_THROW_IF(g4_particle_name.empty(),
                      std::logic_error,
        ...
        if (!_particle_names_map_.empty()) {
          ...
          for (std::map<std::string, std::string>::const_iterator i = _particle_names_map_.begin();
    ...
    {
      if (!is_initialized()) {
        G4Exception("snemo::g4::primary_generator::reset",
                    "InitializationError",
                    FatalException,
    ...
    void
    primary_generator::GeneratePrimaries(G4Event* g4_event_)
    {
      if (!is_initialized()) {
        G4Exception("snemo::g4::primary_generator::GeneratePrimaries",
                    "InitializationError",
                    RunMustBeAborted,
                    "Primary_Generator is not initialized!");
      }

      // THIS IS NOW INVALIDATED: 2011-04-19 FM: moved these bits to
      // 'event_action::BeginOfEventAction' : Clear event data: FIX BUG: 2011-05-10  FM+XG : DO
      // CLEAR THE EVENT DATA MODEL HERE ! clog << "DEVEL: primary_generator::GeneratePrimaries: "
    ...
    void
    primary_generator::_generate_vertex()
    {
      ...
      if (_vertex_generator_->is_time_generator()) {
        ...
        _vertex_generator_->shoot_vertex_and_time(
          mgr.grab_vg_prng(), _current_vertex_, _current_time_);
      } else {
        // Vertex generator:
    ...
    void
    primary_generator::_generate_event(G4Event* g4_event_)
    {
      ...
      for (::genbb::primary_event::particles_col_type::const_iterator i =
        ...
        if (genbb_particle.get_auxiliaries().has_flag(
              mctools::biasing::primary_event_bias::dont_track_this_particle_flag())) {
        ...
        DT_LOG_DEBUG(get_logging_priority(),
                     "Primary particle [#" << particle_counter << "] label = '"
        ...
        if (genbb_particle.has_pdg_code()) {
          DT_LOG_DEBUG(get_logging_priority(),
                       "  Attempting to use primary particle PDG code ["
          ...
          if (g4_particle == 0) {
            ...
            message << "genbb's nucleus particle with PDG code='" << genbb_particle.get_pdg_code()
                    << "') cannot not be processed as a Geant4 particle !";
            G4Exception("snemo::g4::primary_generator::_generate_event",
                        "InvalidArgument",
                        RunMustBeAborted,
                        message.str().c_str());
          }
          ...
          DT_LOG_DEBUG(_logprio(),
                       "Found a PDG particle: " << g4_particle_name << " with G4 PDG mass = "
                                                << g4_particle->GetPDGMass() / CLHEP::MeV << " MeV "
                                                << " and PDG charge = "
                                                << g4_particle->GetPDGCharge());
        } else if (genbb_particle.has_type()) {
          // Support for traditional particle 'types' from genbb_help (extended Geant3 codes):
          DT_LOG_DEBUG(
            _logprio(),
            "  Attempting to use a GENBB particle with type: " << genbb_particle.get_type());
          if (genbb_particle.is_nucleus()) {
            ...
            */
            ...
            bool nucleus_parsing = genbb::primary_particle::label_to_nucleus(
              genbb_particle_label, Z, A, excitation_energy);
            if (!nucleus_parsing) {
              std::ostringstream message;
              message << "genbb's nucleus particle with label='" << genbb_particle_label
                      << "') cannot not be processed as a Geant4 particle !";
              G4Exception("snemo::g4::primary_generator::_generate_event",
                          "InvalidArgument",
                          RunMustBeAborted,
            ...
            g4_particle = G4ParticleTable::GetParticleTable()->GetIon(Z, A, excitation_energy);
            g4_particle_name = g4_particle->GetParticleName();
            _particle_gun_->SetParticleDefinition(g4_particle);
            DT_LOG_TRACE(_logprio(),
                         "Found a nucleus: "
                           << g4_particle_name << " with G4 PDG mass = "
                           << g4_particle->GetPDGMass() / CLHEP::MeV << " MeV "
                           << " (GENBB mass = " << particle_mass / CLHEP::MeV << " MeV)"
                           << " and PDG charge = " << g4_particle->GetPDGCharge()
                           << " (charge set in the gun = "
                           << _particle_gun_->GetParticleCharge() / CLHEP::eplus << " e)");
            ...
            double excitation_energy = 0.0;
            int ion_charge = 0;
            // Fetch the characteristics of the ion (partially ionized) from the GENBB particle
            // label:
            bool ion_parsing = genbb::primary_particle::label_to_ion(
              genbb_particle_label, Z, A, excitation_energy, ion_charge);
            if (!ion_parsing) {
              std::ostringstream message;
              message << "genbb's ion particle with label='" << genbb_particle_label
                      << "') cannot not be processed as a Geant4 particle !";
              G4Exception("snemo::g4::primary_generator::_generate_event",
                          "InvalidArgument",
                          RunMustBeAborted,
            ...
            DT_LOG_DEBUG(_logprio(),
                         "  Assign dynamic charge [" << (ion_charge > 0 ? "+" : "") << ion_charge
                                                     << "e] to the Geant4 ion...");
            ...
            DT_LOG_DEBUG(_logprio(),
                         "Found an ion: "
                           << g4_particle_name << " with G4 PDG mass = " << std::setprecision(15)
                           << g4_particle->GetPDGMass() / CLHEP::MeV << " MeV "
                           << " (GENBB mass = " << std::setprecision(15)
                           << particle_mass / CLHEP::MeV << " MeV)"
                           << " and PDG charge = " << g4_particle->GetPDGCharge() << "e"
                           << " (dynamic charge set in the gun = "
                           << _particle_gun_->GetParticleCharge() / CLHEP::eplus << "e)");
          } else {
            ...
            if (g4_particle_name[0] == '?') {
              ...
              message << "genbb's particle type " << genbb_particle.get_type() << " (with label='"
                      << genbb_particle_label << "') is not recognized as a Geant4 particle !";
              G4Exception("snemo::g4::primary_generator::_generate_event",
                          "InvalidArgument",
                          RunMustBeAborted,
                          message.str().c_str());
            ...
            if (g4_particle == 0) {
              std::ostringstream message;
              message << "snemo::g4::primary_generator::_generate_event: "
                      << "Particle named '" << g4_particle_name
                      << "' is not defined within the Geant4 framework !";
              G4Exception("snemo::g4::primary_generator::_generate_event",
                          "InvalidArgument",
                          RunMustBeAborted,
                          message.str().c_str());
            ...
            DT_LOG_TRACE(_logprio(),
                         "Found the Geant4 '"
                           ...
                           << " with G4 PDG mass = " << g4_particle->GetPDGMass() / CLHEP::MeV
                           << " MeV "
                           << " and PDG charge = " << g4_particle->GetPDGCharge() << "e");
          }
        }
        ...
        if (geomtools::is_valid(_current_vertex_)) {
          ...
          if (!genbb_particle.has_vertex()) {
            std::ostringstream message;
            message << "Particle named '" << g4_particle_name
                    << "' has no valid vertex provided by the event generator !";
            G4Exception("snemo::g4::primary_generator::_generate_event",
                        "InvalidArgument",
                        RunMustBeAborted,
                        message.str().c_str());
        ...
        if (!datatools::is_valid(kinetic_energy)) {
          std::ostringstream message;
          message << "Particle named '" << g4_particle_name
                  << "' has no valid kinetic energy provided by the event generator !";
          G4Exception("snemo::g4::primary_generator::_generate_event",
                      "InvalidArgument",
                      RunMustBeAborted,
                      message.str().c_str());
        ...
        if (_logprio() == datatools::logger::PRIO_TRACE) {
          ...
          std::clog << tag2 << tag << "Type       = " << genbb_particle.get_type() << " ('"
                    << genbb_particle_label << "')" << std::endl;
          std::clog << tag2 << tag << "PDG code   = " << genbb_particle.get_pdg_code() << std::endl;
          std::clog << tag2 << tag << "Geant4 name  = " << g4_particle_name << std::endl;
          std::clog << tag2 << tag
                    << "Time       = " << _particle_gun_->GetParticleTime() / CLHEP::ns << " ns"
                    << std::endl;
          std::clog << tag2 << tag << "Position   = " << _current_vertex_ / CLHEP::mm << " mm"
                    << std::endl;
          std::clog << tag2 << tag << "Momentum   = " << genbb_particle.get_momentum() / CLHEP::keV
                    << " keV" << std::endl;
          if (datatools::is_valid(kinetic_energy)) {
            std::clog << tag2 << last_tag << "Energy     = " << kinetic_energy / CLHEP::keV
                      << " keV (FYI)" << std::endl;
          }
        }
      } // Particle loop
    ...
    std::string
    primary_generator::get_g4_particle_name_from_genbb_particle(
      const ::genbb::primary_particle& p_) const
    {
      ...
      if (p_.is_ion()) {
        // What to do here ? shoud we support it from this method
        // return "ElA[E*]"; // G4 syntax seems to be : He6[0.0] but there is no way to set the ion
        // charge
 ...
 */
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::primary_generator, ocd_)
{
  // The class name :
  ocd_.set_class_name("snemo::g4::primary_generator");
  ...
  ocd_.set_class_documentation(
    "This is Geant4 simulation engine embedded primary generator action. \n");
...
// Registration macro :
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::g4::primary_generator, "snemo::g4::primary_generator")
