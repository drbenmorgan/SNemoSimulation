...
namespace snemo {
  ...
  namespace g4 {
      ...
      if (verboseLevel >= 1) {
        ...
        if (verboseLevel >= 3) {
          ...
          std::clog << "snemo::g4::stepping_verbose::StepInfo " << std::endl
                    ...
                    << "  " << std::setw(20) << "Volume"
                    << "  " << std::setw(10) << "Material" << std::setw(10) << "Process"
                    << std::endl;
        }
        ...
        if (verboseLevel == 2) {
          ...
          if (tN2ndariesTot > 0) {
            ...
            for (size_t lp1 = (*fSecondary).size() - tN2ndariesTot; lp1 < (*fSecondary).size();
                 lp1++) {
              std::clog << "    : " << std::setw(6)
    ...
    void
    stepping_verbose::TrackingStarted()
    {
      CopyState();
