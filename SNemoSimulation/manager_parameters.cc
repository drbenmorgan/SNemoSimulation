...
namespace mctools {
  ...
  namespace g4 {
    ...
    void
    manager_parameters::tree_dump(std::ostream& out_,
                                  ...
                                  bool inherit_) const
    {
      ...
      out_ << indent << datatools::i_tree_dumpable::tag << "interactive = " << interactive
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "g4_visu     = " << g4_visu << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "g4_macro    = '" << g4_macro << "'"
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "logging     = '" << logging << "'"
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "dlls        = " << dlls.size()
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "manager_config_filename = '"
           << manager_config_filename << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "number_of_events        = " << number_of_events << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "number_of_events_modulo = " << number_of_events_modulo << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "mgr_seed                = " << mgr_seed
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "input_prng_states_file  = '"
           << input_prng_states_file << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "init_seed_method        = '"
           << init_seed_method << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "output_prng_states_file = '"
           << output_prng_states_file << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "prng_states_save_modulo = " << prng_states_save_modulo << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "input_prng_seeds_file   = '"
           << input_prng_seeds_file << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "output_prng_seeds_file  = '"
           << output_prng_seeds_file << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "output_data_format      = '"
           << output_data_format << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "output_data_bank_label  = '"
           << output_data_bank_label << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "output_data_file        = '"
           << output_data_file << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "vg_name   = '" << vg_name << "'"
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "vg_seed   = " << vg_seed << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag << "eg_name   = '" << eg_name << "'"
           << std::endl;
      ...
      out_ << indent << datatools::i_tree_dumpable::tag << "output_profiles_activation_rule = '"
           << output_profiles_activation_rule << "'" << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "using_time_stat     = " << using_time_stat << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "forbid_private_hits = " << forbid_private_hits << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "dont_save_no_sensitive_hit_events = " << dont_save_no_sensitive_hit_events
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::inherit_tag(inherit_)
           << "use_run_header_footer = " << use_run_header_footer << std::endl;
      return;
    }

    void
    manager_parameters::set_defaults()
    {
      this->logging = "fatal";
      this->dlls.clear();
    ...
    void
    manager_parameters::setup(const manager_parameters& a_params, manager& a_manager)
    {
      // Setup:
      ...
      DT_THROW_IF(mlogprio == datatools::logger::PRIO_UNDEFINED,
                  std::logic_error,
                  "Invalid logging priority label '" << a_params.logging << "' !");
      ...
      DT_THROW_IF(!boost::filesystem::exists(manager_config_filename),
                  std::runtime_error,
                  "Simulation manager configuration filename '" << manager_config_filename
                                                                << "' does not exist ! ");
      datatools::multi_properties the_configuration(
        "name", "", "Configuration for the Geant4 simulation manager");
      the_configuration.read(manager_config_filename);

      /*** Initialization ***/

      DT_LOG_NOTICE(a_manager.get_logging_priority(), "Initializing the simulation manager...");
