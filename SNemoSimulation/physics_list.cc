...
namespace snemo {
  ...
  namespace g4 {
    ...
    void
    physics_list::production_cuts_info::initialize(double default_cut_gamma_,
                                                   ...
                                                   double default_cut_proton_)
    {
      ...
      DT_THROW_IF(datatools::is_valid(gamma) && gamma <= 0.0,
                  std::domain_error,
                  "Invalid production cut for gamma (" << gamma << ") !");
      DT_THROW_IF(datatools::is_valid(electron) && electron <= 0.0,
                  std::domain_error,
                  "Invalid production cut for electron (" << electron << ") !");
      DT_THROW_IF(datatools::is_valid(positron) && positron <= 0.0,
                  std::domain_error,
                  "Invalid production cut for positron (" << positron << ") !");
      DT_THROW_IF(datatools::is_valid(proton) && proton <= 0.0,
                  std::domain_error,
    ...
    bool
    physics_list::has_geant4_physics_list() const
    {
      return _geant4_physics_list_.get() != 0;
    }

    const G4VModularPhysicsList&
    physics_list::get_geant4_physics_list() const
    {
    ...
    physics_list::grab_geant4_physics_list()
    {
      DT_THROW_IF(!_geant4_physics_list_, std::logic_error, "No Geant4 physics list is defined !");
      return *_geant4_physics_list_.get();
    }
    ...
    bool
    physics_list::has_physics_constructor(const std::string& pc_name_) const
    {
      physics_constructor_dict_type::const_iterator pc_found =
        _physics_constructors_.find(pc_name_);
    ...
    void
    physics_list::initialize(const datatools::properties& config_)
    {
      ...
      if (_using_production_cuts_) {
        ...
        DT_LOG_NOTICE(
          _logprio(),
          "Production cuts low edge energy: " << _production_cuts_low_energy_ / CLHEP::eV << " eV");
        ...
        if (config_.has_key("production_cuts.default_value")) {
          ...
          DT_THROW_IF(_production_cuts_default_value_ <= 0.0,
                      std::domain_error,
        ...
        DT_LOG_NOTICE(_logprio(),
                      "Electron cut set to: " << _production_cuts_values_.electron / CLHEP::mm
                                              << " mm");
        ...
        DT_LOG_NOTICE(_logprio(),
                      "Positron cut set to: " << _production_cuts_values_.positron / CLHEP::mm
                                              << " mm");
        ...
        DT_LOG_NOTICE(_logprio(),
                      "Proton cut set to: " << _production_cuts_values_.proton / CLHEP::mm
                                            << " mm");
    ...
    void
    physics_list::_register_physics_constructors()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
      DT_THROW_IF(_initialized_, std::logic_error, "Already initialized !");
    ...
    void
    physics_list::ConstructParticle()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
    ...
    physics_list::_SetCuts()
    {
      ...
      // G4VUserPhysicsList::SetCutsWithDefault method sets the default cut value for all particle
      // types :
      SetCutsWithDefault();

      double pct_low_edge_energy =
        G4ProductionCutsTable::GetProductionCutsTable()->GetLowEdgeEnergy();
      double pct_high_edge_energy =
        G4ProductionCutsTable::GetProductionCutsTable()->GetHighEdgeEnergy();
      DT_LOG_NOTICE(_logprio(),
                    "Default production cuts low edge energy = " << pct_low_edge_energy / CLHEP::eV
                                                                 << " eV");
      DT_LOG_NOTICE(_logprio(),
                    "Default production cuts high edge energy = "
      ...
      DT_LOG_NOTICE(_logprio(),
                    "Effective production cuts high edge energy = "
                      << pct_high_edge_energy / CLHEP::GeV << " GeV");
      ...
      if (_using_production_cuts_) {
        ...
        // set production cut values for gamma at first and for e- second and next for e+ then
        // proton because some processes for e+/e- need cut values for gamma
    ...
    void
    physics_list::tree_dump(std::ostream& out_,
                            ...
                            bool inherit_) const
    {
      ...
      out_ << std::endl;
      for (physics_constructor_dict_type::const_iterator i = _physics_constructors_.begin();
           i != _physics_constructors_.end();
           i++) {
        ...
        out_ << "'" << pc_entry.name << "' of type '" << pc_entry.id
             << "' (status=" << pc_entry.status << ")";
        out_ << std::endl;
      ...
      out_ << indent << datatools::i_tree_dumpable::tag
           ...
           << std::endl;
      out_ << indent << datatools::i_tree_dumpable::tag
           << "Default production cut value : " << _production_cuts_default_value_ / CLHEP::mm
           << " mm" << std::endl;
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::physics_list, ocd_)
{
...

  {
    ...
    ld << "                                                                      \n";
    ld
      << "See also: http://geant4.cern.ch/support/proc_mod_catalog/physics_lists/useCases.shtml \n";
    ld << "                                                                      \n";
...

  {
    ...
    cpd.set_name_pattern("physics_constructors.names")
      ...
      .set_mandatory(false)
      .set_complex_triggering_conditions(true)
      .set_long_description("This property is not compatible with the ``geant4.physics_list``   \n"
                            "property.                                                          \n")
      .add_example(
        "                                                                            \n"
