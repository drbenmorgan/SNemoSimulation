...
namespace mctools {
  ...
  namespace g4 {
      ...
      if (geom_manager_ != nullptr) {
        ...
        if (! use_materials) {
          ...
          if (use_density_range) {
            ...
            DT_THROW_IF(requested_material_min_density >= requested_material_max_density, std::range_error,
                        "Invalid material density range [" << requested_material_min_density / default_density_unit
                        << " >=" << requested_material_max_density / default_density_unit << "]!");
          }
        }
    ...
    void region_info::tree_dump(std::ostream & out_,
    ...
    void regions_setup::tree_dump(std::ostream & out_,
                                  const std::string & title_,
                                  const std::string & indent_,
                                  bool inherit_) const
    {
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::region_info, ocd_)
{
  ...
  ocd_.set_class_documentation("The Geant4 simulation engine/manager is able to define *region* objects       \n"
  ...
  ocd_.set_configuration_hints("Typical configurations for regions follows.                        \n"
                               "                                                                   \n"
