...
namespace snemo {
  ...
  namespace g4 {
    ...
    bool
    region_info::is_initialized() const
    {
    ...
    void
    region_info::initialize(const datatools::properties& config_,
                            const geomtools::manager* geom_manager_)
    {
      ...
      if (geom_manager_ != nullptr) {
        ...
        materials::material::state_type requested_material_state =
          materials::material::STATE_UNKNOWN;
        if (!use_materials && !use_density_range) {
          if (config_.has_key("material.state")) {
            ...
            DT_THROW_IF(requested_material_state != materials::material::STATE_UNKNOWN,
                        std::logic_error,
                        "Invalid material state '" << state_label << "'!");
          }
        }
        ...
        const materials::manager* mat_mgr_ref = nullptr;
        {
          ...
          DT_THROW_IF((use_density_range || use_state) && mat_mgr_ref == 0,
                      std::logic_error,
        ...
        for (geomtools::logical_volume::dict_type::const_iterator ilogical =
          ...
          for (const std::string& log_name_regex : the_log_volumes_regexp) {
            ...
            if (boost::regex_match(log_name, e1)) {
              // std::cerr << " ************* DEVEL ************ Logical '" << log_name << "'
              // matches the region regex '" << log_name_regex << "' \n";
              add_it = true;
            }
          }
          ...
          if (!mr.empty()) {
            if (use_materials) {
              ...
              DT_THROW_IF(!mat_mgr_ref->has_material(effective_mat_ref),
                          std::logic_error,
                          "Unknown material '" << effective_mat_ref
              ...
              if (use_density_range) {
                ...
                // Check if the state of the material of the logical volume matches the requested
                // one:
    ...
    bool
    region_info::is_active() const
    {
      return is_initialized() && _logical_ids_.size();
    }

    void
    region_info::add_logical_volume(const std::string& logical_id_, bool allow_duplicate_)
    {
      DT_THROW_IF(!allow_duplicate_ && _logical_ids_.count(logical_id_),
                  std::logic_error,
                  "Logical volume '" << logical_id_ << "' already belong to the region '"
                                     << get_name() << "'!");
    ...
    void
    region_info::remove_logical_volume(const std::string& logical_id_)

    {
      DT_THROW_IF(!has_logical_volume(logical_id_),
                  std::logic_error,
    ...
    bool
    region_info::has_logical_volume(const std::string& logical_id_) const
    {
      return _logical_ids_.count(logical_id_) > 0;
    }

    const std::set<std::string>&
    region_info::get_logical_ids() const
    {
      return _logical_ids_;
    }

    std::set<std::string>&
    region_info::grab_logical_ids()
    {
      return _logical_ids_;
    }

    void
    region_info::tree_dump(std::ostream& out_,
    ...
    void
    regions_setup::set_geometry_manager(const geomtools::manager& geom_mgr_)
    {
    ...
    regions_setup::get_geometry_manager() const
    {
    ...
    bool
    regions_setup::has_region(const std::string& region_id_) const
    {
      return _region_infos_.find(region_id_) != _region_infos_.end();
    }

    void
    regions_setup::initialize(const datatools::properties& config_)
    {
      DT_THROW_IF(_geom_manager_ == 0, std::logic_error, "Missing referenced geometry manager!");
    ...
    const region_infos_dict_type&
    regions_setup::get_region_infos() const
    {
      return _region_infos_;
    }

    region_infos_dict_type&
    regions_setup::grab_region_infos()
    {
      return _region_infos_;
    }

    void
    regions_setup::tree_dump(std::ostream& out_,
                             const std::string& title_,
                             const std::string& indent_,
                             bool inherit_) const
    {
