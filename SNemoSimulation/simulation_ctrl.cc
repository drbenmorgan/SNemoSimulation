...
namespace mctools {
  namespace g4 {
    ...
    void
    simulation_ctrl::set_simulation_manager(manager& a_simulation_manager)
    {
      simulation_manager = &a_simulation_manager;
    }

    void
    simulation_ctrl::start()
    {
      simulation_thread =
        new boost::thread(boost::bind(&simulation_ctrl::start_simulation_run, this));
    }

    void
    simulation_ctrl::start_simulation_run()
    {
      {
        boost::mutex::scoped_lock lock(*event_mutex);
    ...
    void
    simulation_ctrl::set_stop_requested()
    {
      stop_requested = true;
    }

    bool
    simulation_ctrl::is_stop_requested() const
    {
