...
namespace snemo {
  namespace g4 {
    ...
    run_action::io_work_type::io_work_type() { return; }
    ...
    bool
    run_action::is_initialized() const
    {
      return _initialized_;
    }

    bool
    run_action::using_run_header_footer() const
    {
      return _use_run_header_footer_;
    }

    void
    run_action::set_use_run_header_footer(bool a_new_value)
    {
    ...
    manager&
    run_action::grab_manager()
    {
      return *_manager_;
    }

    const manager&
    run_action::get_manager() const
    {
      return *_manager_;
    }

    bool
    run_action::save_data() const
    {
      return _save_data_;
    }

    bool
    run_action::has_number_of_events_modulo() const
    {
      return _number_of_events_modulo_ > NUMBER_OF_EVENTS_MODULO_NONE;
    }

    void
    run_action::set_number_of_events_modulo(int modulo_)
    {
    ...
    int
    run_action::get_number_of_events_modulo() const
    {
      return _number_of_events_modulo_;
    }

    int32_t
    run_action::get_number_of_saved_events() const
    {
      return _number_of_saved_events_;
    }

    int32_t
    run_action::get_number_of_processed_events() const
    {
      return _number_of_processed_events_;
    }

    void
    run_action::increment_number_of_processed_events()
    {
    ...
    void
    run_action::increment_number_of_saved_events()
    {
    ...
    void
    run_action::reset_number_of_saved_events()
    {
    ...
    void
    run_action::reset_number_of_processed_events()
    {
    ...
    void
    run_action::set_output_file(const std::string& a_filename)
    {
      DT_THROW_IF(
        is_initialized(), std::logic_error, "Cannot change output  file name ! Object is locked !");
    ...
    bool
    run_action::has_out_module() const
    {
      return _io_work_.get() != 0 && _io_work_->out_module.get() != 0;
    }

    const dpp::output_module&
    run_action::get_out_module() const
    {
    ...
    bool
    run_action::has_brio_writer() const
    {
      return _io_work_.get() != 0 && _io_work_->brio_writer.get() != 0;
    }

    const brio::writer&
    run_action::get_brio_writer() const
    {
    ...
    void
    run_action::register_event_action(event_action& event_action_)
    {
    ...
    void
    run_action::initialize(const datatools::properties& a_config)
    {
      // Fetch parameters:
      ...
      if (search_file_directives) {
        ...
        if (!using_brio && a_config.has_key("file.compression")) {
          ...
          if (ok) {
            ...
            DT_THROW_IF(
              true, std::logic_error, "Invalid compression mode '" << compression << "' !");
          }
        }
    ...
    void
    run_action::dump(std::ostream& a_out) const
    {
    ...
    void
    run_action::BeginOfRunAction(const G4Run* a_run)
    {
      std::cout << std::flush;
      std::cerr << std::flush;
      std::clog << std::flush;
        ...
        if (_output_data_format_ == io_utils::DATA_FORMAT_BANK) {
          ...
          if (using_brio) {
            ...
            DT_LOG_NOTICE(_logprio(),
                          "Opening brio serialization output data file '" << output_file_name
                                                                          << "'...");
            DT_LOG_DEBUG(_logprio(),
            ...
            DT_LOG_DEBUG(_logprio(),
                         "Brio writer has store '"
                           << store << "' : " << _io_work_->brio_writer->has_store(store));
            ...
            DT_LOG_NOTICE(_logprio(),
                          "Opening datatools serialization output data file '" << output_file_name
                                                                               << "'...");
    ...
    void
    run_action::EndOfRunAction(const G4Run* a_run)
    {
    ...
    void
    run_action::store_data(const mctools::simulated_data& esd_)
    {
      _store_data(esd_);
      return;
    }
    ...
    void
    run_action::_store_data(const mctools::simulated_data& esd_)
    {
      if (get_manager().using_time_stat()) {
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::run_action, ocd_)
{
...

  {
    ...
    cpd.set_name_pattern("file.using_run_header_footer")
      .set_terse_description("Flag to activate the saving of special informations in run header "
                             "and footer data structure")
      .set_traits(datatools::TYPE_BOOLEAN)
      .set_mandatory(false)
      .set_triggered_by_flag("file.save")
