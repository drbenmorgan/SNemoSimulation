...
// Ourselves:
#include <SNemoSimulation/em_physics_constructor.h>
...
// This project:
#include <SNemoSimulation/processes/em_extra_models.h>
...
namespace snemo {
  ...
  namespace g4 {
    ...
    struct em_physics_constructor::_work_type_ {
      std::map<std::string, ::snemo::g4::processes::em_extra_model> em_extra_models;
      G4EmConfigurator em_configurator;
      int em_configurator_verbosity;
      ::snemo::g4::processes::em_model_factory em_model_fact;
    };
    ...
    em_physics_constructor::region_deexcitation_type::region_deexcitation_type(bool fluo_,
                                                                               bool auger_,
                                                                               bool pixe_)
    {
      fluorescence = fluo_;
    ...
    bool
    em_physics_constructor::region_deexcitation_type::is_activated() const
    {
      return fluorescence || auger || pixe;
    }

    bool
    em_physics_constructor::region_deexcitation_type::is_fluorescence() const
    {
      return fluorescence;
    }

    bool
    em_physics_constructor::region_deexcitation_type::is_auger() const
    {
      return auger;
    }

    bool
    em_physics_constructor::region_deexcitation_type::is_pixe() const
    {
      return pixe;
    }
    ...
    const std::string em_physics_constructor::EM_PIXE_ELECTRON_MODEL_PROTON_ANALYTICAL =
      "Analytical";
    const std::string em_physics_constructor::EM_PIXE_ELECTRON_MODEL_PROTON_EMPIRICAL = "Empirical";

    const std::string&
    em_physics_constructor::get_em_model() const
    {
      return _em_model_;
    }

    bool
    em_physics_constructor::is_em_standard() const
    {
      return _em_model_ == EM_MODEL_STANDARD;
    }

    bool
    em_physics_constructor::is_em_low_energy_livermore() const
    {
      return _em_model_ == EM_MODEL_LOW_ENERGY_LIVERMORE;
    }

    bool
    em_physics_constructor::is_em_low_energy_penelope() const
    {
    ...
    em_physics_constructor::_work_type_&
    em_physics_constructor::_grab_work_()
    {
      if (_work_.get() == 0) {
    ...
    void
    em_physics_constructor::initialize(const datatools::properties& config_,
                                       physics_constructor_dict_type& /*dict_*/)
    {
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Electron/positron energy loss set to: " << _em_electron_ionization_);

      if (config_.has_key("em.electron.multiple_scattering")) {
        _em_electron_multiple_scattering_ =
          config_.fetch_boolean("em.electron.multiple_scattering");
      }
      DT_LOG_DEBUG(
        _logprio(),
        "Electron/positron multiple scattering set to: " << _em_electron_multiple_scattering_);
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Electron/positron multiple scattering use distance to boundary: "
                     << _em_electron_ms_use_distance_to_boundary_);
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Electron/positron step limiter set to: " << _em_electron_step_limiter_);
      ...
      if (config_.has_key("em.deexcitation.pixe.electron_cross_section_model")) {
        ...
        DT_LOG_DEBUG(
          _logprio(),
          "EM PIXE e+/e- cross-section model set to : " << _em_pixe_electron_cross_section_model_);
      }
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Muon multiple scattering set to: " << _em_muon_multiple_scattering_);
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Other multiple scattering set to: " << _em_other_multiple_scattering_);
    ...
    void
    em_physics_constructor::_setup_em_extra_models_configurator(
      const datatools::properties& config_)
    {
      ...
      if (config_.has_key("em.configurator.verbosity")) {
        ...
        if (verbosity_label == "mute") {
          ...
          DT_THROW(std::logic_error,
                   "Invalid EM configurator verbosity '" << verbosity_label << "'!");
        }
      }
      ...
      if (extra_models_config.has_key("labels")) {
        ...
        for (int i = 0; i < (int)em_xmodel_labels.size(); i++) {
          ...
          extra_models_config.export_and_rename_starting_with(
            extra_model_config, em_xmodel_prefix.str(), "");
          {
            snemo::g4::processes::em_extra_model dummy;
            _grab_work_().em_extra_models[em_xmodel_label] = dummy;
          }
          snemo::g4::processes::em_extra_model& em_xmodel =
      ...
      for (std::map<std::string, ::snemo::g4::processes::em_extra_model>::const_iterator
             i = _grab_work_().em_extra_models.begin();
           i != _grab_work_().em_extra_models.end();
           i++){
        const std::string & em_xmodel_label = i->first;
        const ::snemo::g4::processes::em_extra_model & em_xmodel = i->second;
    ...
    void
    em_physics_constructor::_set_defaults()
    {
      // Process:
    ...
    void
    em_physics_constructor::tree_dump(std::ostream& out_,
                                      ...
                                      bool inherit_) const
    {
      ...
      out_ << indent << datatools::i_tree_dumpable::tag
           << "Gamma conversion             : " << (_em_gamma_conversion_ ? "Yes" : "No")
           << std::endl;
      ...
      out_ << indent << datatools::i_tree_dumpable::tag
           << "Positron annihilation : " << (_em_positron_annihilation_ ? "Yes" : "No")
           << std::endl;
      ...
      out_ << indent << datatools::i_tree_dumpable::tag
           << "Ion user special cuts : " << (_em_ion_user_special_cuts_ ? "Yes" : "No")
           << std::endl;
      ...
      out_ << indent << datatools::i_tree_dumpable::tag
           << "Muon user special cuts : " << (_em_muon_user_special_cuts_ ? "Yes" : "No")
           << std::endl;
    ...
    void
    em_physics_constructor::ConstructParticle()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
      DT_THROW_IF(!is_initialized(), std::logic_error, "Not initialized !");
    ...
    void
    em_physics_constructor::ConstructProcess()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
    ...
    void
    em_physics_constructor::_ConstructExtraModels()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
      ...
      DT_LOG_DEBUG(_logprio(), "Apply EM extra models to the Geant4 EM configurator...");
      for (std::map<std::string, ::snemo::g4::processes::em_extra_model>::const_iterator i =
        ...
        // Declare extra models to the embedded Geant4 EM configurator:
        DT_LOG_NOTICE(_logprio(),
                      "Declare extra model '" << em_xmodel_label
      ...
      DT_LOG_DEBUG(_logprio(),
                   "Activating declared extra EM models through the EM configurator...");
    ...
    void
    em_physics_constructor::_ConstructEMDeexcitation()
    {
      DT_LOG_TRACE_ENTERING(_logprio());
        ...
        if (particle_name == "gamma") {
          ...
          if (is_em_low_energy_livermore()) {
            ...
            if (the_compton_scattering) {
              G4LivermoreComptonModel* the_livermore_compton_model = new G4LivermoreComptonModel();
            ...
            if (the_rayleigh_scattering) {
              G4PenelopeRayleighModel* the_penelope_rayleigh_model = new G4PenelopeRayleighModel();
            ...
            if (the_compton_scattering) {
              G4PenelopeComptonModel* the_penelope_compton_model = new G4PenelopeComptonModel();
          ...
          if (_em_gamma_user_special_cuts_) {
            ++process_rank;
            DT_LOG_NOTICE(_logprio(),
                          "Add user special cuts limiter for '" << particle_name << "'");
          ...
          if (_em_electron_multiple_scattering_) {
            ...
            ++process_rank;
            DT_LOG_NOTICE(_logprio(),
                          "Add e+/e- multiple scattering for '" << particle_name << "'");
          ...
          if (_em_ion_multiple_scattering_) {
            ...
            ++process_rank;
            DT_LOG_NOTICE(_logprio(),
                          "Add hadron multiple scattering for '" << particle_name << "'");
 ...
 */
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::g4::em_physics_constructor, ocd_)
{
  // The class name :
  ocd_.set_class_name("snemo::g4::em_physics_constructor");
...

  {
    ...
    cpd.set_name_pattern("em.deexcitation.auger")
      ...
      .set_mandatory(false)
      .set_default_value_boolean(false)
      .set_long_description("Auger effect is only activated if ``em.deexcitation.fluorescence``\n"
                            "is also activated.                                                \n")
...
// Registration macro for class 'snemo::g4::em_physics_constructor' :
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::g4::em_physics_constructor,
                               "snemo::g4::em_physics_constructor")
